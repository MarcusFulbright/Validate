...
/**
 * Turns out validating emails is really hard. This was pulled directly from Aura.Filter with no modifications.
 *
 *
...
class Email
{
        ...
        switch ($this->token) {
            ...
            case Email::STRING_HTAB:
                if (($this->token === Email::STRING_CR)
                    && ((++$this->pos === $this->rawLength) || ($this->email[$this->pos] !== Email::STRING_LF))
                ) {
                    // Fatal error
            ...
            case Email::STRING_AT:
                ...
                } elseif (($this->contextPrior === Email::CONTEXT_COMMENT)
                    || ($this->contextPrior === Email::CONTEXT_FWS)
                ) {
                    // http://tools.ietf.org/html/rfc5322#section-3.4.1
            ...
            default:
                ...
                if ($this->endOrDie) {
                    ...
                    if (($ord < 33)
                        || ($ord > 126)
                        || ($ord === 10)
                        || (!is_bool(strpos(Email::STRING_SPECIALS, $this->token)))
                    ) {
                        // Fatal error
                        $this->returnStatus[] = Email::ERR_EXPECTING_ATEXT;
                    }
    ...
    protected function parseComponentDomain()
    {
        ...
        switch ($this->token) {
            ...
            case Email::STRING_HTAB:
                if (($this->token === Email::STRING_CR)
                    && ((++$this->pos === $this->rawLength)
                    || ($this->email[$this->pos] !== Email::STRING_LF))
                ) {
                    // Fatal error
                    $this->returnStatus[] = Email::ERR_CR_NO_LF;
                    break;
        ...
        switch ($this->token) {
            ...
            case Email::STRING_CLOSESQBRACKET:
                if ((int) max($this->returnStatus) < Email::DEPREC) {
                    ...
                    $pattern = '/\\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9]'
                    .'[0-9]?)$/';
                    if (preg_match($pattern, $addressliteral, $matchesIP) > 0) {
                    ...
                    if ($index === 0) {
                        ...
                        if ($index === false) {
                            ...
                            if ($index !== strrpos($IPv6, Email::STRING_DOUBLECOLON)) {
                                ...
                                if ($index === 0 || $index === (strlen($IPv6) - 2)) {
                                    // RFC 4291 allows :: at the start or end of an address with 7 other groups 4
                                    //in addition
                                    $max_groups++;
                                }
                        ...
                        // Revision 2.7: Daniel Marschall's new IPv6 testing strategy
                        if ((substr($IPv6, 0, 1) === Email::STRING_COLON)
                            && (substr($IPv6, 1, 1) !== Email::STRING_COLON)
                        ) {
                            // Address starts with a single colon
                            $this->returnStatus[] = Email::RFC5322_IPV6_COLONSTRT;
                        } elseif ((substr($IPv6, -1) === Email::STRING_COLON)
                            && (substr($IPv6, -2, 1) !== Email::STRING_COLON)
                        ) {
    ...
    protected function parseFinal()
    {
        ...
        if ((int) max($this->returnStatus) < Email::RFC5322) {
            ...
            } elseif (strlen($this->parseData[Email::COMPONENT_LOCALPART] . Email::STRING_AT
                    . $this->parseData[Email::COMPONENT_DOMAIN]) > 254) {
                // http://tools.ietf.org/html/rfc5321#section-4.1.2
                //   Forward-path = Path
                //
                //   Path = "<" [ A-d-l ":" ] Mailbox ">"
    ...
    protected function checkDns()
    {
        // Check DNS?
        if ($this->checkDns
            && ((int) max($this->returnStatus) < Email::DNSWARN) && function_exists('dns_get_record')
        ) {
...
// Not using checkdnsrr because of a suspected bug in PHP 5.3 (http://bugs.php.net/bug.php?id = 51844)
            $result = @dns_get_record($this->parseData[Email::COMPONENT_DOMAIN], DNS_MX);
